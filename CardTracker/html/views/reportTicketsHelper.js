...
var comboPeriodos = new comboPeriodos2();
...
function initReportTicketsHTML () {
	comboUsers.username= username;
	combo.holder = username;
	//ConnectionString esta definido en GlobalVariables.js
	combo.connectionString = ConnectionString;
	...
	comboPeriodos.destinatioDiv = "#comboPeriodos";
		
	gridPurchases.destinationDiv = "#grid";
	gridPurchases.connectionString = ConnectionString;
	gridPurchases.gridType = "FULL"; // <<<< DIFERENCIA con addTicket.html
    ...
    setTimeout(function() {
		...
		comboUsers.createCombo();
		var dropdownlist = $("#comboUsers").data("kendoDropDownList");
		dropdownlist.bind("change",onComboUsersChangeValueRT); 

    	
    	//* inicializar combo de cuentas *//
        combo.createCombo();
		dropdownlist = $("#comboBox").data("kendoDropDownList");
		dropdownlist.bind("change",onComboAccountsChangeValueRT); // <<<<<<< diferencia
		
		comboCurrency.createCombo();
		
		gridPurchases.hash = "nada";
		gridPurchases.purchased_by = this.username;
		gridPurchases.orderBy = "";
//		gridPurchases.createGrid();
...
/** FUNCIONES reportTickets ---------------------------------------------   **/

function onComboUsersChangeValueRT(event) {
	...
	var x = document.getElementById("divMovimientos");
	x.style.display = "none";
	
	
}

function onComboAccountsChangeValueRT (event) {
	...
	comboPeriodos.credit_card = dataItem.acc_id;
	comboPeriodos.createCombo();
	
	dropdownlist = $("#comboPeriodos").data("kendoDropDownList");
	dropdownlist.bind("change",onComboPeriodosChangeValueRT); 
	

}
function onComboPeriodosChangeValueRT (event) {
	...
	gridPurchases.credit_card = dataItem.acc_id;
	gridPurchases.period = dataItem2.period;
	gridPurchases.createGrid();
...
function showAddTicket() {
	...
	var x = document.getElementById("divAddTicket");
	x.style.display = "block";
